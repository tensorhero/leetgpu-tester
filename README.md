...
```bash
git clone https://github.com/bootcraft-cn/leetgpu-tester
cd leetgpu-tester
go build .
...
docker pull ghcr.io/bootcraft-cn/leetgpu-tester:latest
docker run --rm --user $(id -u):$(id -g) -v "$(pwd):/workspace" ghcr.io/bootcraft-cn/leetgpu-tester:latest -s map -d /workspace
```
...
```bash
#!/bin/bash
docker run --rm --user $(id -u):$(id -g) -v "$(pwd):/workspace" ghcr.io/bootcraft-cn/leetgpu-tester:latest \
  -s "${1:-map}" -d /workspace
```
